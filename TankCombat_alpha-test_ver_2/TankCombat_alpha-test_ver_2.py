...
class USA_Tanks:
    ...
    def ReturnTankInfo(self):
        TankInfo = [self.Name, self.Hp, self.Speed, self.MG_I_List, self.SG_I_List, RsiBm]
        return TankInfo
...
#||전차              = Exa_Tanks("전차 이름"         ,         분류,체력,속도,     주포 정보, 부포 정보, 폭압저항력)
#|||WW2_Early_USAm_Tank_List
M3A1_Stuart          = USA_Tanks("M3A1 스튜어트"     , '[경전차]'  , 100,  58,     weC37mmM6,      None, 2000)
M4A1_Sherman         = USA_Tanks("M4A1 셔먼"         , '[중형전차]', 210,  39,     weC75mmM3,      None, 3000)
M3_Lee               = USA_Tanks("M3 리"             , '[중전차]'  , 215,  39,     weC37mmM5, weC75mmM2, 3000)
M10_GMC              = USA_Tanks("M10 GMC"           , '[구축전차]', 190,  39,     weC76mmM7,      None,  300)
#|||WW2_Middle_USAm_Tank_List
M18_GMC              = USA_Tanks("M18 GMC"           , '[경전차]'  , 210,  80,     wmC76mmM1,      None,  300)
M26_Pershing         = USA_Tanks("M26 퍼싱"          , '[중형전차]', 300,  48,   wmC90mmM3_o,      None, 3500)
M4A3E2_76_W_Jumbo    = USA_Tanks("M4A3E2 점보셔먼"   , '[중전차]'  , 375,  42,     wmC76mmM1,      None, 3400)
M36B2_o              = USA_Tanks("M36B2"             , '[구축전차]', 220,  40,   wmC90mmM3_s,      None,  500)
#|||WW2_Late_USAm_Tank_List
M41A1_WalkerBulldog_o= USA_Tanks("M41A1 워커불독"    , '[경전차]'  , 300,  72,    wlC76mmM32,      None, 2000)
M46_Patton_o         = USA_Tanks("M46 패튼"          , '[중형전차]', 400,  48, wlC90mmM3A1_o,      None, 4000)
T34                  = USA_Tanks("T34"               , '[중전차]'  , 550,  35,   wlC120mmT53,      None, 4000)
M56_Scorpion         = USA_Tanks("M56 스콜피온"      , '[구축전차]', 280,  45,    wlC90mmM54,      None,  400)
#|||Korean_War_USAm_Tank_List
M41A1_WalkerBulldog_s= USA_Tanks("M41A1 워커불독"    , '[경전차]'  , 300,  72,    kwC76mmM32,      None, 2000)
M46_Patton_s         = USA_Tanks("M46 패튼"          , '[중형전차]', 400,  48, kwC90mmM3A1_s,      None, 4000)
M4A3E8_76_W_HVSS     = USA_Tanks("M4A3E8 이지에잇"   , '[중전차]'  , 400,  48,   kwC76mmM1A2,      None, 3500)
M36B2_s              = USA_Tanks("M36B2"             , '[구축전차]', 220,  40,   kwC90mmM3_t,      None,  500)
...
def Show_M3A1_Stuart():
    ...
    for name, val in globals().items():
        if val is weC37mmM6:
            print(f"|주포 구경 & 이름: {name[3:7]} {name[7:9]}")
    print(f"|재장전 시간: {W2E_UT_L[0].MG_I_List[0]}초")
    print(f"|사용 가능한 포탄:")
    for i in range(len(W2E_UT_L[0].MG_I_List) - 1):
        if W2E_UT_L[0].MG_I_List[i+1][1] == 'APCBC':
            SaySelCls = '저저항피모철갑탄'
        print(f''' |포탄 이름: {W2E_UT_L[0].MG_I_List[i+1][0]}
 |포탄 종류: {W2E_UT_L[0].MG_I_List[i+1][1]}({SaySelCls})
  |관통력: {W2E_UT_L[0].MG_I_List[i+1][2]}mm
  |피해량: {W2E_UT_L[0].MG_I_List[i+1][3]}
    ''')
    print("1. 시작 | 0. 돌아가기")
...
def Show_M4A1_Sherman():
    ...
    for i in range(len(W2E_UT_L[1].MG_I_List) - 1):
        print(i)
    print("1. 시작 | 0. 돌아가기")
    Check = input("번호를 입력하세요: ")
    if Check == '1':
